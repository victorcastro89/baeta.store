	...
	class WOODMART_Widget_Price_Filter extends WPH_Widget {
		...
		function __construct() {
			...
			$args['fields'] = array(
				...
				array(
					...
					'description' => esc_html__( 'May increase this widget performance if you have a lot of products in your sore.', 'woodmart' ),
				),
			);
		...
		private function generate_price_links( $min, $max, $min_price, $max_price, $show_ranges ) {
			...
			if( $need_more ) {
				...
				if( $this->check_range($step_min, $max, $show_ranges) ) {
					$links[] = array(
						'href' 	=> $href,
						'title' => $step_title,
						'class' => $step_class
