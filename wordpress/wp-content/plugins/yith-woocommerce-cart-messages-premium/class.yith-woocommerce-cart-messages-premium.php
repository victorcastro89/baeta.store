...
if ( ! class_exists( 'YWCM_Cart_Messages_Premium' ) ) {
	...
	class YWCM_Cart_Messages_Premium extends YWCM_Cart_Messages {
		...
		public function enqueue_styles_scripts() {
			...
			wp_enqueue_script( 'ywcm_frontend', YITH_YWCM_ASSETS_URL . '/js/ywcm-frontend' . YITH_YWCM_SUFFIX . '.js', array( 'jquery', 'wc-add-to-cart-variation' ), YITH_YWCM_VERSION, true );
		...
		public function get_minimum_amount_args( $message ) {
			...
			$products_to_exclude = (array) get_post_meta( $message->ID, '_ywcm_minimum_amount_products_exclude', true );
		...
		public function is_valid_extra_check( $value, $message_id ) {
			...
			if ( 'simple_message' == $message_type ) {
				$show_in_products = get_post_meta( $message_id, '_ywcm_simple_message_show_in_products', true );
